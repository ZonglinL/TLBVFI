...
def parse_args_and_config():
    ...
    parser.add_argument('--frame0', type=str, default="/home/zonglin/benchmark/PerVFI/results/FILM_extreme/prev/", help='previous frame')
    parser.add_argument('--frame1', type=str, default="/home/zonglin/benchmark/PerVFI/results/FILM_extreme/next/", help='next frame')
    parser.add_argument('--frame', type=str, default="/home/zonglin/benchmark/PerVFI/results/FILM_extreme/gt/", help='next frame')
...
def main():
    ...
    dp = 0
    for i in range(310):
        frame0_path = os.path.join(args.frame0,f"{i}.png")
        frame1_path = os.path.join(args.frame1,f"{i}.png")
        frame_gt_path = os.path.join(args.frame,f"{i}.png")
        ...
        frame0 = transform(Image.open(frame0_path)).cpu().unsqueeze(0)
        frame1 = transform(Image.open(frame1_path)).cpu().unsqueeze(0)
        frame_gt = transform(Image.open(frame_gt_path)).cpu().unsqueeze(0)
        I = encode(frame0,frame1,model,frame_gt)
        I_ = encode(frame0,frame1,model,None)

        mean += (I.mean() - I_.mean())
        sum1 += ((I - I.mean())**2).sum()
        sum2 += ((I_ - I_.mean())**2).sum()
        dp += len(I.flatten())
